...
function renderStatic (staticDef, viewsPath, distPath, data, knownHelpers) {
    ...
    return ensureDirectories(distPath, destPath)
        .then(() => renderTemplate(viewsPath, staticDef.view, data, knownHelpers))
        .then(content => writeFile(destFileName, content));
}

function renderDynamicFile (view, targetPath, data, context) {
    return renderTemplate(context.viewsPath, view, data, context.knownHelpers)
        .then((content) => {
            const destPath = getDestPath(targetPath);

            if (!context.bucket) {
                const destFileName = path.join(context.distPath, destPath);
                return ensureDirectories(context.distPath, destPath)
                    .then(() => writeFile(destFileName, content));
            }
